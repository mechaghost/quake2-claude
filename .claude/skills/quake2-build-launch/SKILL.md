...
## Discovering Kex cvars and commands

The engine's 186 commands + 579 cvars are dumped into `docs/kex-cmds.txt` and `docs/kex-cvars.txt`, generated by `dump-kex-vocab.bat`. See `docs/kex-vocab-guide.md` for curated highlights (bot cvars especially — `bot_minClients`, `bot_skill`, `bot_aim_disabled`, `bot_pause`, etc. are all real and useful for A/B testing the Claude bot vs. different levels of engine opposition). When in doubt about a cvar, grep that file first instead of guessing.
